	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 9.
				...
				for(GoodsVO vo:list)
				{
					...
					out.write("<a href=MainServlet?mode=5&no=\"+vo.getNo()+\"&type=3>"); // # : 임의 설정
				...
				if(startPage>1)
				{
					out.write("<li><a href=MainServlet?page="+(startPage-1)+"&mode=3>&lt;</a></li>"); // &lt; 이동 버튼
				}
				for(int i=startPage;i<=endPage;i++)
				{
					out.write("<li "+(i==curpage?"class=active":"")+"><a href=MainServlet?page="+i+"&mode=3>"+i+"</a></li>"); // active : 색깔이 바뀐다.
				}
				if(endPage>1)
				{
					out.write("<li><a href=MainServlet?page="+(endPage+1)+"&mode=3>&gt;</a></li>"); // &gt; 이동 버튼
				} 
				out.write("</ul>");
				out.write("</div>");
				out.write("</div>");
