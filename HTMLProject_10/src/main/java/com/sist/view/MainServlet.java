...
public class MainServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.write("</head>");
		out.write("<body>");
		RequestDispatcher rd=request.getRequestDispatcher("MenuServlet"); // MenuServlet을 include해라.
		rd.include(request, response);	// 모든 서블릿이 request를 공유한다.
		out.write("<div style=\"height:30px\"></div>");
		out.write("<div class=container>");
		// 8.
		rd=request.getRequestDispatcher(name);
		rd.include(request, response);	// <jsp:include page="">
		////
		out.write("</div>");
		out.write("</body>");
		out.write("</html>");
