	...
	$(function() {
		$('.returnBtn').click(function() {
			...
			if(confirm('책을 반납하시겠습니까?')===true){
				location.href='../mypage/bookLoanReturn.do?isbn='+isbn
			}else{
				return false
			}
		})
...
<!-- 도서대출정보 메뉴 include -->
	...
	<div style="width: 100%;padding: 0;">
		대출현황 <font color="red">${totalcount }</font>건
		...
		<c:if test="${list_size gt 0 }">
			...
			<c:forEach var="vo" items="${list }">
				<tr>
					...
					<td width="15%" class="text-center">대출중</td>
					<td width="10%" class="text-center">
						<span class="btn btn-sm btn-warning returnBtn" data-isbn="${vo.isbn }">반납</span>
					</td>
				</tr>
		...
		<div class="text-center">
			<ul class="pagination">
				<c:if test="${startBlockNum>1 }">
					<li><a href="../mypage/bookloaninfo_main.do?page=${curpage gt 1 ? startBlockNum-1 : curpage }&searchDate=${searchDate }&searchType=${searchType }&search=${search }&date1=${date1 }&date2=${date2 }">&lt;</a></li>
				</c:if>
				<c:forEach var="i" begin="${startBlockNum }" end="${endBlockNum }">
					<li ${curpage eq i ? "class=active" : "" }><a href="../mypage/bookloaninfo_main.do?page=${i }&searchDate=${searchDate }&searchType=${searchType }&search=${search }&date1=${date1 }&date2=${date2 }">${i }</a></li>
				</c:forEach>
				<c:if test="${endBlockNum<totalpage }">
					<li><a href="../mypage/bookloaninfo_main.do?page=${curpage lt totalpage ? endBlockNum+1 : curpage }&searchDate=${searchDate }&searchType=${searchType }&search=${search }&date1=${date1 }&date2=${date2 }">&gt;</a></li>
				</c:if>
			</ul> 
		</div>
	</div>
